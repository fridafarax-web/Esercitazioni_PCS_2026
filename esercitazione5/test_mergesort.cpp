...
int main(){
	cout << endl << "Si testa l'algoritmo Mergesort su 100 vettori di dimensioni randomiche da 1 a 8000 riempiti di valori randomici da -10000 a +10000 interi, double e float." <<endl;
	double tempo=0;
	randfiller rf;
	...
	for(int i=0;i<dimensioni.size();i++){
		...
		if(t_1 == 1){
			cout << "Mergesort int Test "<< i+1 << " EXIT FAILURE"<<endl;
			return EXIT_FAILURE;
		}
		int t_2 = test_sort<double>(dimensioni[i],-10000, 10000);
		if(t_2 == 1){
			cout << "Mergesort double Test "<< i+1 << " EXIT FAILURE"<<endl;
			return EXIT_FAILURE;
		}
		int t_3 = test_sort<float>(dimensioni[i],-10000, 10000);
		if(t_3 == 1){
			cout << "Mergesort float Test "<< i+1 << " EXIT FAILURE"<<endl;
			return EXIT_FAILURE;
		}
		tempo = tempo + tc.toc();
	}
